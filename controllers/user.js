...
router.get('/cart',function(req,res){
	...
	if(req.session.CartProduct!=null){
		...
		res.layout('layouts/user/header', {title:"User Panel"}, {content:{block:"user/cart",data:{catPro:req.session.CartProduct,totalPrice:totalPrice,totalQuantity:totalQuantity}}});
	}else{
		res.layout('layouts/user/header', {title:"User Panel"}, {content:{block:"user/cart",data:{catPro:null,totalPrice:0,totalQuantity:0}}});
	}
...
router.get('/cart/:id',function(req,res){
	productModel.getProductById(req.params.id,function(result){
		...
		//var itemsInCart = req.session.CartProduct.length;
...
router.get('/order',function(req,res){
	res.layout('layouts/user/loggedHeader', {title:"User Panel"}, {content:{block:"user/order", data:{msg:""}}});
});
...
router.post('/order',function(req,res){
	...
	if(req.session.CartProduct!=null){
	    ...
	    orderModel.insert(order,function(flag){
	    		if(flag){
	    			...
	    			orderModel.getLastOrderIdByCutomerId(req.session.loggedUser[0].id,function(result){
						...
						res.layout('layouts/user/loggedHeader', {title:"User Panel"}, {content:{block:"user/order", data:{msg:"Order placed successfully!!"}}});
		...
		res.layout('layouts/user/loggedHeader', {title:"User Panel"}, {content:{block:"user/order", data:{msg:"Opps!! Your cart is empty."}}});
	}
