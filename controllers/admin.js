...
router.get('/',function(req,res){
	res.layout('layouts/admin/header', {title:"Admin Panel"}, {content:{block:"admin/index", data:{name:"Admin"}}});
});
			...
			if(request.files.pic){
			 	...
			 	if(request.files.pic.mimetype=="image/jpeg" || request.files.pic.mimetype=="image/png"){
						...
						productModel.updateProductById(Updatedproduct, function(flag){
							if(flag)
							{
								response.layout('layouts/admin/header', {title:"Admin Panel"}, {content:{block:"admin/editProduct", data:{msg:"Product updated succesfully!! ",categories: result,product: result2}}});
							}
							else
							{
			 		...
			 		response.layout('layouts/admin/header', {title:"Admin Panel"}, {content:{block:"admin/editProduct", data:{msg:"Invalid image!!",categories: result,product: result2}}});
			 	}
			}else{
				response.layout('layouts/admin/header', {title:"Admin Panel"}, {content:{block:"admin/editProduct", data:{msg:"Image not selected!!",categories: result,product: result2}}});
			}
		});
...
router.get('/deliveredOrder/',function(req,res){
	...
	orderModel.getAllDeliveredOrder(function(result){
		res.layout('layouts/admin/header', {title:"Admin Panel"}, {content:{block:"admin/deliveredOrder", data:{deliveredOrders:result}}});
	});
